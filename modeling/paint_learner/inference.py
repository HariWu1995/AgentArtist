...
from modeling.paint_learner.baseline.Renderer.model import FCN
from modeling.paint_learner.baseline.DRL.actor import ResNet
from modeling.paint_learner.brushstrokes import draw_curve
from modeling.paint_learner.utilities import preprocess, postprocess, make_gif
...
if __name__ == "__main__":
    ...
    # Load models
    painter_ckpt_path = './checkpoints/learn_to_paint/default/actor.pkl'
    renderer_ckpt_path = './checkpoints/learn_to_paint/default/renderer.pkl'
    ...
    image_path = "./samples/MidJourney-1er-prix-Colorado-State-Fair.png"
    image_size = 768
    image = cv2.imread(image_path, cv2.IMREAD_COLOR)
    image = cv2.resize(image, (image_size, image_size))
    image = cv2.copyMakeBorder(image, (768-384)//2, (768-384)//2, 0, 0, cv2.BORDER_CONSTANT)

    # Run pipeline
    D = int(image_size / WIDTH)
    out_dir = f'./results/midjourney_{image_size}'
    with torch.no_grad():
