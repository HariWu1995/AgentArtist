...
class PainterModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.meta_brushes = torch.cat([brush_large_vertical, brush_large_horizontal], dim=0)
    ...
    def set_input(self, input_dict):
        ...
        with torch.no_grad():
            ...
            foregrounds = foregrounds.view(self.opt.batch_size // 4, self.opt.used_strokes, 3, self.patch_size * 2, self.patch_size * 2).contiguous()

            alphas = morphology.Erosion2d(m=1)(alphas)
            alphas = alphas.view(self.opt.batch_size // 4, self.opt.used_strokes, 3, self.patch_size * 2, self.patch_size * 2).contiguous()
    ...
    def gaussian_w_distance(self, param_1, param_2):
        ...
        trace_12 = torch.sqrt(trace_12[..., 0, 0] + trace_12[..., 1, 1] + \
                2 * torch.sqrt(trace_12[..., 0, 0] * trace_12[..., 1, 1] - \
    ...
    def optimize_parameters(self):
        ...
        with torch.no_grad():
            ...
            for i in range(self.gt_param.shape[0]):
                ...
                pred_param_broad = self.pred_param[i].unsqueeze(1).contiguous().repeat(1, valid_gt_param.shape[0], 1)
                valid_gt_param_broad = valid_gt_param.unsqueeze(0).contiguous().repeat(self.pred_param.shape[1], 1, 1)
