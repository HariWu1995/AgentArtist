...
def param2img_serial(
    ...
    Args:
        ...
        decision: a 01 tensor with shape 
                batch size x patch along height dimension x patch along width dimension x n_stroke_per_patch
        meta_brushes: a tensor with shape 2 x 3 x meta_brush_height x meta_brush_width.
                The 1st slice on the batch dimension denotes vertical brush and the 2nd one denotes horizontal brush.
    ...
    def partial_render(this_canvas, patch_coord_y, patch_coord_x, stroke_id):
        ...
        selected_foregrounds = torch.zeros(selected_param.shape[0], 3, patch_size_y, patch_size_x, device=this_canvas.device)
        ...
        if selected_param[selected_decision, :].shape[0] > 0:
            selected_foregrounds[selected_decision, :, :, :], \
                 selected_alphas[selected_decision, :, :, :] = param2stroke(selected_param[selected_decision, :], 
                                                                            patch_size_y, patch_size_x, meta_brushes)
    ...
    """
        ...
        Args:
            ...
            decision: a 01 tensor with shape 
                    batch size x patch along height dimension x patch along width dimension x n_stroke_per_patch
            meta_brushes: a tensor with shape 2 x 3 x meta_brush_height x meta_brush_width.
                    The 1st slice on the batch dimension denotes vertical brush and the 2ond one denotes horizontal brush.
            cur_canvas: a tensor with shape batch size x 3 x H x W,
                    where H and W denote height and width of padded results of original images.
    ...
    with torch.no_grad():
        ...
        for layer in range(0, K+1):
            ...
            img_temp = img_patch.unsqueeze(1).contiguous().repeat(1, stroke_num, 1, 1, 1)\
                                .view(img_patch.shape[0] * stroke_num, 3, patch_size, patch_size).contiguous()
            color = F.grid_sample(img_temp, 2 * grid - 1, align_corners=False)\
                     .view(img_patch.shape[0], stroke_num, 3).contiguous()
            ...
            if serial:
                final_result = param2img_serial(param, decision, meta_brushes, final_result, frame_dir, False, original_h, original_w)
            else:
        ...
        img    = F.pad(   img, [patch_size // 2, patch_size // 2, patch_size // 2, patch_size // 2, 0, 0, 0, 0])
        result = F.pad(result, [patch_size // 2, patch_size // 2, patch_size // 2, patch_size // 2, 0, 0, 0, 0])
        ...
        param[..., 2:4] = param[..., 2:4] / 2
        if serial:
            final_result = param2img_serial(param, decision, meta_brushes, final_result, frame_dir, True, original_h, original_w)
        else:
            final_result = param2img_parallel(param, decision, meta_brushes, final_result)
