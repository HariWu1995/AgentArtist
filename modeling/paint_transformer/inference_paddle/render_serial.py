# !/usr/bin/env python3
"""
codes for oil-painting style transfer.
"""
import time
...
def get_single_stroke_on_full_image_A(x_id, y_id, valid_foregrounds, valid_alphas, param, original_img, 
    ...
    tmp_alpha = tmp_alpha[:, :, render_size_y // 10:-render_size_y // 10, 
                                render_size_x // 10:-render_size_x // 10]
...
def stroke_net_predict(img_patch, result_patch, patch_size, net_g, stroke_num):
    ...
    img_temp = img_patch.unsqueeze(1).tile([1, stroke_num, 1, 1, 1])\
                        .reshape([img_patch.shape[0] * stroke_num, 3, patch_size, patch_size])
    color = nn.functional.grid_sample(img_temp, 2 * grid - 1, align_corners=False)\
...
def render_serial(original_img, net_g, meta_brushes):
    ...
    with paddle.no_grad():
        ...
        for layer in range(0, K + 1):
            ...
            wA_xid_list, wA_yid_list, wA_fore_list, wA_alpha_list, \
            wA_error_list, wA_params = get_single_layer_lists(param, decision, original_img, 
                                                                render_size_x, render_size_y, h, w, 
            ...
            wB_xid_list, wB_yid_list, wB_fore_list, wB_alpha_list, \
            wB_error_list, wB_params = get_single_layer_lists(param, decision, expand_img, 
                                                                render_size_x, render_size_y, h, w, 
                                                                meta_brushes, dilation, erosion, stroke_num)
            ...
            for ii in sort_list:
                ...
                if ii < numA:
                    ...
                    params = wA_params[ii]
                    tmp_foreground, \
                    tmp_alpha = get_single_stroke_on_full_image_A(x_id, y_id, 
                                                                valid_foregrounds, valid_alphas, params, 
                                                                original_img, render_size_x, render_size_y, patch_x, patch_y)
                else:
                    ...
                    params = wB_params[ii - numA]
                    tmp_foreground, \
                    tmp_alpha = get_single_stroke_on_full_image_B(x_id, y_id, 
                                                                valid_foregrounds, valid_alphas, params, 
                                                                original_img, render_size_x, render_size_y, patch_x, patch_y)
