...
from utils import morphology, util
...
class PainterModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--used_strokes', type=int, default=8, help='actually generated strokes number')
        parser.add_argument('--num_blocks', type=int, default=3, help='number of transformer blocks for stroke generator')
    ...
    def set_input(self, input_dict):
        ...
        with torch.no_grad():
            ...
            foregrounds = foregrounds.view(self.opt.batch_size // 4, old_used_strokes, 3, self.patch_size * 2, self.patch_size * 2).contiguous()
            alphas      =      alphas.view(self.opt.batch_size // 4, old_used_strokes, 3, self.patch_size * 2, self.patch_size * 2).contiguous()
            ...
            if self.controler_paras_num != 0:
                ...
                gt_param[:, :, 5:5 + self.controler_paras_num] = (gt_param[:, :, 5:5 + self.controler_paras_num] - 0.5) * 0.4
            ...
            foregrounds = foregrounds.view(self.opt.batch_size, self.opt.used_strokes, 3, self.patch_size, self.patch_size).contiguous()
            alphas      =      alphas.view(self.opt.batch_size, self.opt.used_strokes, 3, self.patch_size, self.patch_size).contiguous()
    ...
    def optimize_parameters(self):
        ...
        with torch.no_grad():
            ...
            for i in range(self.gt_param.shape[0]):
                ...
                pred_param_broad = self.pred_param[i, :, :self.d_shape0].unsqueeze(1).contiguous().repeat(1, valid_gt_param.shape[0], 1)
                valid_gt_param_broad = valid_gt_param[:, :self.d_shape0].unsqueeze(0).contiguous().repeat(self.pred_param.shape[1], 1, 1)
