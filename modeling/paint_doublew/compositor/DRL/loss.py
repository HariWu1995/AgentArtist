...
class PixelLoss(nn.Module):
    ...
    def forward(self, canvas, gtruth, ignore_color=True):
        if gtruth.max() > 2:
            canvas = canvas / 255.0
            gtruth = gtruth / 255.0
        if ignore_color:
            canvas = torch.mean(canvas, dim=1)
            gtruth = torch.mean(gtruth, dim=1)
        loss = (torch.abs(canvas-gtruth)).mean(dim=(1,2))
        return loss
...
class VGGStyleLoss2(torch.nn.Module):
    ...
    def forward(self, input, target):
        ...
        for i in range(1, 5):
            ...
            loss += ((stylized_feats_mean - s_feats_mean)**2).mean(dim=(1,2,3)) \
                  + ((stylized_feats_std  - s_feats_std )**2).mean(dim=(1,2,3))
        return loss
...
class StyleReward(torch.nn.Module):
    ...
    def __init__(self, image_path='style.jpg'):
        ...
        self.style_img = read_img(image_path).to(DEVICE)
