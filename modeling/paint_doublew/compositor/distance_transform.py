...
def differentialble_distance_transform(image, kernel_size, c=0.3):
    ...
    unfold_nopad = nn.Unfold(kernel_size=kernel_size, stride=1, padding=0)
    unfold_pad   = nn.Unfold(kernel_size=kernel_size, stride=1, padding=half_size)
    fold = nn.Fold(output_size=(image.size(-2), image.size(-1)), kernel_size=1, stride=1)
